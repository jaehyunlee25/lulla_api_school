...
export default async function handler(req, res) {
  ...
  // #3. 데이터 처리
  // #3.1. 작업
  setBaseURL('sqls/invite/accept/carer'); // 끝에 슬래시 붙이지 마시오.
  ...
  try {
    ...
    return ERROR(res, {
      id: 'ERR.invite.accept.carer.3.2.2',
...
async function main(req, res) {
  ...
  if (userId !== inv.user_id)
    return ERROR(res, {
      id: 'ERR.invite.accept.carer.3.2.3',
  ...
  const qKid = await QTS.newKid.fQuery({ kidName });
  if (qKid.type === 'error')
  ...
  const qSMFG5 = await QTS.getSMFG5.fQuery({ memberId, classId });
