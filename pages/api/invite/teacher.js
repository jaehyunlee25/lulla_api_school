...
const baseUrl = 'sqls/invite/teacher'; // 끝에 슬래시 붙이지 마시오.
...
async function main(req, res) {
  ...
  const { phone, role_name: roleName, class_id: classId, type } = invitation;
  ...
  if (qMIUI.message.rows.length === 0)
    return ERROR(res, {
      resultCode: 400,
      id: 'ERR.invite.teacher.3.2.3',
      message: 'no such member',
    });
  const { school_id: schoolId, grade } = qMIUI.message.rows[0];

  // #3.3. 선생님 초대는 1. 원장, 2. 관리자만이 가능하다.
  if (grade > 2)
    return ERROR(res, {
      resultCode: 401,
      id: 'ERR.invite.teacher.3.3.1',
      message: '선생님 초대장을 생성할 권한이 없습니다.',
    });
