...
const baseUrl = 'sqls/member/getMembers'; // 끝에 슬래시 붙이지 마시오.
let EXEC_STEP = 0;
...
async function main(req, res) {
  ...
  const { member_id: memberId, grade: userGrade } = req.body;
  ...
  if (qMIUI.message.rows.length === 0)
    return ERROR(res, {
      resultCode: 400,
      id: 'ERR.member.getMembers.3.2.2',
  ...
  if (grade > 2)
    return ERROR(res, {
      resultCode: 400,
      id: 'ERR.member.getMembers.3.2.3',
      message: '멤버 목록을 조회할 권한이 없습니다.',
    });

  const qMems = await QTS.getMembers.fQuery(baseUrl, {
    schoolId,
    grade: userGrade,
  });
  if (qMems.type === 'error')
  ...
  const members = qMems.message.rows;
