...
const baseUrl = 'sqls/drug/update'; // 끝에 슬래시 붙이지 마시오.
let EXEC_STEP = 0;
...
async function main(req, res) {
  ...
  if (qAnn.type === 'error')
    return qAnn.onError(res, '3.2.3.1', 'searching drug');
  ...
  if (qChat.type === 'error')
    return qChat.onError(res, '3.5.1', 'creating drug');
  if (qChat.message.rows.length === 0)
    return ERROR(res, {
      resultCode: 400,
      id: 'ERR.drug.index.3.4.1',
  ...
  if (fileList && fileList.length > 0) {
    ...
    if (qFiles.type === 'error')
      return qFiles.onError(res, '3.7.1', 'searching drug file');
  ...
  if (qData.type === 'error')
    return qData.onError(res, '3.10.1', 'searching drug');
