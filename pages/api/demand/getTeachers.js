...
const baseUrl = 'sqls/demand/getTeachers'; // 끝에 슬래시 붙이지 마시오.
export default async function handler(req, res) {
  ...
  try {
    ...
    return ERROR(res, {
      id: 'ERR.demand.getTeachers.3.2.2',
...
async function main(req, res) {
  ...
  if (qMIUI.message.rows.length === 0)
    return ERROR(res, {
      resultCode: 400,
      id: 'ERR.demand.getTeachers.3.2.3',
  ...
  if (grade > 2)
    return ERROR(res, {
      resultCode: 401,
      id: 'ERR.demand.getTeachers.3.3.1',
  ...
  if (confirmed) {
    ...
    if (qGet.type === 'error')
      return qGet.onError(res, '3.4.1', 'searching demand');
  } else {
    ...
    if (qGet.type === 'error')
      return qGet.onError(res, '3.4.2', 'searching demand');
  }
  ...
  return RESPOND(res, {
    teachers,
    resultCode: 200,
    message: '선생님 요청장을 성공적으로 반환하였습니다.',
  });
}
